...
class DetonationBasedMergePolicy():
  """DCPSO merge policy that makes the number of particles vary throughout execution.
  Particles that are close to other particles get deleted.
  When particles continue to drop, a 'detonation' occurs - particles spawn within a radius
  of the global best.
  If another detonation occurs in the same swarm, the detonation radius will double.
  Detonation restores the number of particles - the number of particles that spawn in
  from the detonation will be such that we start with the original swarm size.
    ...
    self.detonation_radius - float
        -the initial radius of a detonation, or in other words, the area around the global best where particles will spawn in
    self.particle_deletion_threshold - float
        -the minimum distance a particle can have with another particle before being deleted.
    self.detonation_value - triggers detonation when the number of particles drops below this value
    """

  def __init__(self,stagnation_threshold,patience,detonation_radius,particle_deletion_threshold,detonation_value):
    ...
    self.pdt = particle_deletion_threshold
    self.detonation_value
  ...
  def merge_stagnated_swarms(self,stagnation_list,algorithm):
          ...
          for swarm in stagnation_list:
              ...
              new_velocity[:,tracker:tracker+swarm.dims] = 0 #swarm.velocity
