...
class RandomMergePolicy():
  """MCPSO merge policy that merges stagnated swarms with a randomly chosen (unstagnated) swarm
    Attributes:
    ...
    self.stagnation_list - list
        -after stagnation calculations, stagnated swarms are held here for further processing later
  """
  def __init__(self,stagnation_threshold,patience,reset_velocity = False):
  ...
  def deal_with_stagnated_swarms(self,algorithm):
      ...
      while self.stagnation_list: #merge whenever there is more than one swarm stagnating
          ...
          #choose a random swarm and merge a stagnated swarm
          random_swarm_idx = np.random.randint(len(algorithm.swarms))
          new_swarm = self.merge_stagnated_swarms([self.stagnation_list[0], algorithm.swarms[random_swarm_idx]],algorithm)

          #remove the original swarms we just merged
          algorithm.swarms.pop(random_swarm_idx)
